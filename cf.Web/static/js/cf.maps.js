        ...
        switch (geoType) {
            case "Point":
                ...
                if (tempCoord != null) {
                    ...
                    infoBoxContents[tempCoord] = "<a href='" + link + "'>" + title + "<br /><img src='" + displayImgUrl + "' /></a>";
...
function displayInfobox(e) {
    ...
    if (pin != null) {
        ...
        var options = {
            htmlContent: "<span class='mapInfoBox'>" + infoBoxContents[location] + "</span>",
            visible: true, showPointer: true,
            offset: new Microsoft.Maps.Point(0, pin.getHeight() + 40), zIndex: 999
        };

        if (pinInfobox != null) {
            infoBoxLayer.remove(pinInfobox);
            if (Microsoft.Maps.Events.hasHandler(pinInfobox, 'mouseleave')) { Microsoft.Maps.Events.removeHandler(pinInfobox.mouseLeaveHandler); }
            if (Microsoft.Maps.Events.hasHandler(pinInfobox, 'mouseenter')) { Microsoft.Maps.Events.removeHandler(pinInfobox.mouseEnterHandler); }
            pinInfobox = null;
        }
...
function hideInfobox(e) { if (pinInfobox != null) { pinInfobox.setOptions({ visible: false }); }  }
function startInfoboxTimer(e) { if (pinInfobox.pinTimer != null) { clearTimeout(pinInfobox.pinTimer); } pinInfobox.pinTimer = setTimeout(timerTriggered, 300); }
function stopInfoboxTimer(e) { if (pinInfobox != null && pinInfobox.pinTimer != null) { clearTimeout(pinInfobox.pinTimer); } }
function mapViewChange(e) { stopInfoboxTimer(e); hideInfobox(e); }
function pinMouseOver(e) { displayInfobox(e); }
function pinMouseOut(e) { startInfoboxTimer(e); }
function pinInfoboxMouseLeave(e) { hideInfobox(e); }
function pinInfoboxMouseEnter(e) { stopInfoboxTimer(e); }
function timerTriggered(e) { hideInfobox(e); }
